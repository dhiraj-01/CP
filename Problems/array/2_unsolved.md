...
### Solution
```
Observation 1 : N < 1e4, A[i] <= 1000
so expected time / space = O(N * 1000)

observations 2 : all a[i % k] elements are same 
a[i] = a[i + k] = a[i + 2 * k] ...
...
ending conditions => if( curXor == 0 && id == k)

Observation 3 : here, we can optimise 1024 iterations per step..
